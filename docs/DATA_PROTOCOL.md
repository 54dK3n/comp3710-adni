...
## Implemented Optional Training Augmentation

The `dataset/augmentation.py` light profile applies small random rotations and translations only after audited training membership and per-slice source-byte verification. The default remains `none`. Light defaults are ±5 degrees and ±3% translation along each axis. Transformations are sampled independently per slice, run on a fixed canvas with bilinear interpolation and black fill, and do not estimate statistics from any patient population.

`dataset/loaders.py` receives an explicit role. Nontrivial augmentation is accepted only for `train` rows from `development`; evaluation roles reject it. `engine/training.py` passes the profile only to the train loader. `engine/prediction.py` validates saved augmentation metadata but always constructs an unaugmented validation loader. Model selection continues to use unaugmented early-stop scans, followed by outer validation after checkpoint selection.

Private per-worker random generators preserve the original model random stream. Reproduction requires the same seed, worker configuration, versions, and execution settings. All transform settings are written to the version 2 checkpoint and run configuration. Legacy version 1 checkpoints, which used no augmentation, remain supported. Augmentation is an experiment to evaluate within development; it does not guarantee better performance or certify biological invariance.
...
The audit entry point is `adni_splits.py`, backed by the unchanged `dataset/splits.py` implementation, which requires Python 3.9 or newer and Pillow. It implements **data indexing, splitting, and integrity auditing**. CNN and ConvNeXt-Tiny development-fold training and checkpoint inference are implemented in `train.py` and `predict.py`; see the [README](../README.md) for their dependencies and commands. Both models use fixed grayscale intensity scaling, fresh random initialization in every fold, training-only loss weights, an independent early-stop subset, and outer validation after checkpoint selection. ConvNeXt-Tiny is implemented without external pretrained weights; its LayerNorm operates within each image and does not fit population statistics. Stochastic depth affects training only. Checkpoints identify their architecture, and prediction uses that saved architecture with the original manifest fingerprint. Final refitting, confidence calibration, and final-test evaluation are not yet implemented. The full protocol remains a requirement for those future stages.
...
The purpose of this protocol is to establish leakage-prevention boundaries that can be inspected and reproduced. A split should only be described as having passed the implemented data checks after the real server run and review of its results. The user has reported a successful audit for the current split, as documented above. The CNN and ConvNeXt execution paths are covered by synthetic CPU tests. Three five-fold CNN repetitions have undergone local log, prediction and manifest review; the initial repetition also had checkpoint review before the real-run weights were removed locally. ConvNeXt real-data performance remains to be measured on the course server. Original-image and metadata verification remains server-side; the future final-calibration/test workflow has not yet been implemented or verified.
